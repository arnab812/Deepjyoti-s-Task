...
const mongoNotebook = require('./mongoNotebook');

//fetching all data
router.get('/', async (req, res) => {
    try {
        const notes = await mongoNotebook.find();
        res.json(notes);
...
router.get('/:id', async (req, res) => {
    try {
        const notes = await mongoNotebook.findById(req.params.id)
        res.json(notes)
    } catch (err) {
        res.send(err)
    }
